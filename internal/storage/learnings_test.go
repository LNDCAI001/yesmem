...
func TestSetTaskTypeBatch(t *testing.T) {
	s := mustOpen(t)

	// Insert three test learnings
	id1 := newTestLearning(t, s)
	id2 := newTestLearning(t, s)
	id3 := newTestLearning(t, s)

	// Set task_type to stale for first two
	if err := s.SetTaskTypeBatch([]int64{id1, id2}, TaskTypeStale); err != nil {
		t.Fatalf("SetTaskTypeBatch: %v", err)
	}

	// Verify id1 and id2 have task_type=stale, id3 does not
	for _, tc := range []struct {
		...
		l, err := s.GetLearning(tc.id)
		if err != nil {
			t.Fatalf("GetLearning(%d): %v", tc.id, err)
		}
		if l.TaskType != tc.wantTaskType {
			t.Errorf("id %d: task_type = %q, want %q", tc.id, l.TaskType, tc.wantTaskType)
		}
	}
}

func TestSetTaskTypeBatch_Empty(t *testing.T) {
	s := mustOpen(t)
	if err := s.SetTaskTypeBatch(nil, TaskTypeStale); err != nil {
		t.Fatalf("SetTaskTypeBatch(nil): %v", err)
	}
	if err := s.SetTaskTypeBatch([]int64{}, TaskTypeStale); err != nil {
		t.Fatalf("SetTaskTypeBatch([]): %v", err)
	}
}

func TestSetTaskTypeBatch_Superseded(t *testing.T) {
	s := mustOpen(t)

	id := newTestLearning(t, s)

	// First resolve the learning
	if err := s.ResolveBatch([]int64{id}, "test resolve"); err != nil {
		t.Fatalf("ResolveBatch: %v", err)
	}

	// Now try to set task_type — should not affect superseded item
	if err := s.SetTaskTypeBatch([]int64{id}, TaskTypeStale); err != nil {
		t.Fatalf("SetTaskTypeBatch: %v", err)
	}
	...
	if l.TaskType == TaskTypeStale {
		t.Error("superseded learning should not have been marked stale")
	}
	if l.SupersededBy == nil {
		t.Error("learning should still be superseded")
	}
}

func TestReactivateBatchCleanup(t *testing.T) {
	s := mustOpen(t)

	batchReason := "batch-cleanup: stale >30 days"

	// Insert and resolve two learnings with batch-cleanup reason
	id1 := newTestLearning(t, s)
	id2 := newTestLearning(t, s)
	// Insert and resolve one with a different reason (should not be reactivated)
	id3 := newTestLearning(t, s)

	if err := s.ResolveBatch([]int64{id1, id2}, batchReason); err != nil {
		t.Fatalf("ResolveBatch id1,id2: %v", err)
	}
	if err := s.ResolveBatch([]int64{id3}, "manual resolve"); err != nil {
		t.Fatalf("ResolveBatch id3: %v", err)
	}

	// Verify all three are superseded
	for _, id := range []int64{id1, id2, id3} {
		l, err := s.GetLearning(id)
		if err != nil {
			t.Fatalf("GetLearning(%d): %v", id, err)
		}
		if l.SupersededBy == nil {
			t.Errorf("id %d: expected superseded, got nil", id)
		}
	}

	// Count should return 2 (only batch-cleanup items)
	preCount, err := s.CountBatchCleanup()
	if err != nil {
		t.Fatalf("CountBatchCleanup: %v", err)
	}
	if preCount != 2 {
		t.Errorf("CountBatchCleanup: expected 2, got %d", preCount)
	}

	count, err := s.ReactivateBatchCleanup()
	if err != nil {
		t.Fatalf("ReactivateBatchCleanup: %v", err)
	}
	if count != 2 {
		t.Errorf("expected 2 reactivated, got %d", count)
	}

	// id1 and id2 should be active again
	for _, id := range []int64{id1, id2} {
		...
		if l.SupersededBy != nil {
			t.Errorf("id %d: expected superseded_by=NULL after reactivation, got %v", id, *l.SupersededBy)
		}
		if l.SupersedeReason != "" {
			t.Errorf("id %d: expected empty supersede_reason, got %q", id, l.SupersedeReason)
		}
	}

	// id3 (manual resolve) should remain superseded
	l3, err := s.GetLearning(id3)
	if err != nil {
		t.Fatalf("GetLearning(%d): %v", id3, err)
	}
	if l3.SupersededBy == nil {
		t.Error("id3: manually resolved learning should still be superseded")
	}

	// Second call should return 0 (nothing left to reactivate)
	count2, err := s.ReactivateBatchCleanup()
	if err != nil {
		t.Fatalf("ReactivateBatchCleanup second call: %v", err)
	}
	if count2 != 0 {
		t.Errorf("second call: expected 0, got %d", count2)
	}
}

func TestReactivateBatchCleanup_Empty(t *testing.T) {
	s := mustOpen(t)
	count, err := s.ReactivateBatchCleanup()
	if err != nil {
		t.Fatalf("ReactivateBatchCleanup on empty: %v", err)
	}
	if count != 0 {
